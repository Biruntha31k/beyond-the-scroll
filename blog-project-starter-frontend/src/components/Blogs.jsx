...
function Blogs() {
    ...
    useEffect(() => {
        ...
        auth.onAuthStateChanged(function (user) {
            if (user) {
                if (user.uid === "CiZjJ5qIQyO4Qd5SVhKx2FHHqVn2") {
                    setAdmin(true)
                }
                else {
                    setAdmin(false)
    ...
    return (
        <div className="blog-section py-14">
            <h2 className="text-center text-5xl font-serif mb-14">Latest  <span className='text-[#c57ff4] '>Blogs</span> 📚</h2>
            {/* Blog creation form */}
            {admin ?
                <div className="blog-creation-form mb-8  " style={{ width: "80%", margin: "auto" }}>
                    <form onSubmit={handleNewBlogSubmit} className="flex flex-col gap-4 ">
                        <input
                            ...
                            className="p-2 border rounded border-[#c57ff4] focus:outline-none"
                            required
                        />
                        <textarea
                            ...
                            required
                        />
                        <button type="submit" className="bg-[#FF0000] text-white p-2 mb-5 rounded hover:bg-[#c57ff4]">
                            Add Blog
                        </button>
                    </form>
                </div> : ""}
